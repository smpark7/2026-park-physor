...
modeling every channel discretely, accounting for 3-D heat conduction, tracking delayed neutron
precursor movement, and modeling steady state and various transient scenarios.
...
These modeling capabilities have been verified in code-to-code verification studies for
...
Moltres. These 1-D quantities are projected to the 3-D Moltres problem domain. Consequently, the
fluid temperature and delayed neutron source are radially uniform within each fuel channel.
...
The \gls{MSR} model in this study is a preliminary design being developed for the \gls{MSR}
...
multiphysics \gls{MSR} analysis tools of varying model fidelity.
...
Figure \ref{fig:geometry} shows the horizontal and vertical cross section views of the 3-D
quarter-core \gls{MSR} geometry. The quarter-core geometry consists of 284 regular fuel channels
and two central, annular salt half-channels. The fuel channel centroids form a
...
fuel salt and surrounded radially by the same vessel wall. Table \ref{table:dimensions} lists some
of the key physical dimensions of the \gls{MSR} core model.
...
re-entering the core through a downcomer. Out-of-core components were adapted from an existing
\gls{SAM} \gls{MSRE} model \cite{hu_fy21_2021} openly available in the Virtual Test Bed Repository
\cite{giudicelli_virtual_2023}. The present work assumes a uniform gamma heat source in the
graphite regions equal to 7.8~\% of total fission heat produced.
...
We ran a pseudo-transient simulation towards steady state with explicit coupling between Moltres
(steady-state) and SAM (transient). Moltres ran the $k$-eigenvalue neutronics calculation
concurrently with the steady-state heat conduction calculation in a single \gls{PJFNK} solve every
time step.

The simulation ran on two AMD EPYC 9754 128-core processors and
took approximately 30 wall seconds per time step. 
...
\section{SIMULATION RESULTS \& DISCUSSION$^1$}
...
within this paper. The authors acknowledge the SAM development team at Argonne National Laboratory
