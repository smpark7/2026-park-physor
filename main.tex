...
temperature reactivity feedback and the advection of delayed neutron precursors. This work
...
leverages computationally efficient system-level thermal-hydraulics while retaining
channel-specific 3-D spatial resolution for precursor tracking and reactivity feedback in the core.
We present
...
\keywords{molten salt reactor, multiphysics, multiscale, MOOSE, MSRE}
...
tasks will require both experimental data and reliable simulation data from physics-based reactor
analysis tools. Physics-based tools must demonstrate robustness and predictive capabilities over a
wide range of \gls{MSR} simulation scenarios to produce reliable
training and reference data for reduced-order models. To that end, parallel efforts by the authors
focus on code-to-code verification exercise between VERA \cite{graham_multiphysics_2021} and
GeN-Foam \cite{fiorina_gen-foam_2022} through a series of \gls{MSR} progression problems
\cite{gentry_graphite-moderated_2025} based on the \gls{MSRE} reactor design. The VERA approach
couples its high-fidelity 2-D Method of Characteristics (MOC)/1-D nodal $P_N$ neutron transport
solver MPACT to a system thermal-hydraulics code tailored for channel-type \gls{MSR} analysis.
GeN-Foam provides several multiphysics configuration options through a range of neutronics (neutron
diffusion, $SP_3$, and $S_N$) and computational fluid dynamics models.

In this work, we introduce a third multiphysics solver that bridges between the differing model
fidelities of VERA and GeN-Foam for improved comparative analysis. It is a new multiscale 3-D/1-D
multiphysics \gls{MSR} analysis tool
built by linking two \gls{MOOSE} \cite{harbour_40_2025} applications: Moltres
...
framework, the Moltres-SAM coupling leverages \gls{SAM}, an efficient nuclear system code supported
by the \gls{NEAMS} program with specific features tailored for advanced reactor safety analysis.
This tool also benefits from recent development work on Moltres for \gls{MSR} analysis
...
Moltres for this work is its integration within the \gls{MOOSE} ecosystem. As a \gls{MOOSE}-based
tool, Moltres benefits from highly scalable mesh and solver routines and ease of 
developing multiphysics coupling with \gls{SAM}.
...
optimization, safety analyses, and licensing support. \gls{SAM} developers have implemented and
tested several advanced features applicable to salt-cooled reactor analysis
...
metrics. The second \gls{SAM} model coupled to the Griffin reactor physics code and
modeled porous media flow in a homogenized 2-D axisymmetric \gls{MSRE} model
\cite{jaradat_multiphysics_2023}. Both approaches modeled the ex-core salt loop using serially
connected 1-D pipe regions forming a closed loop with the core region.
...
3-D Moltres model. Consequently, the model sacrifices radial fidelity within each fuel channel. We
expect this to have a small impact on local flux distributions and reactivity feedback but minimal
impact on the overall distributions and feedbacks. These impacts will be investigated in future
comparisons with GeN-Foam.
...
material properties. Salt flows upwards through the core and flows through a pump and a heat
exchanger before re-entering the core through a downcomer.
...
Moltres applies monotone cubic interpolation on two-group material
...
The 3-D model applies a cosine gamma heat source, digitally sampled from Figure 14.2 of an
\gls{MSRE} report \cite{haubenreich_msre_1964}, in the graphite regions that is normalized to
produce 7.8~\% of total fission heat. Ideally, the \gls{MSR} salt-graphite interfaces should have
heat transfer coefficients that depend on local state variables to account for developing flow and
internal heat generation. However, due to existing software constraints in \gls{SAM}, the current
model uses a constant heat transfer correlation for laminar flow ($Nu=4.36$).
...
$T_\text{s}=824.8$~K. We derived $h$ analytically by defining the core inlet temperature
...
qualitatively with the \gls{DNP} distributions in the \gls{MSRE} benchmark report
...
Future efforts will incorporate control rod modeling and investigate asymmetric transients, such as
blocked fuel channels and natural circulation events. Additionally, we will explore integrating
this tool into a digital twin framework to generate training data and act as a digital shadow to
reconstruct past incidents, forecast future behavior, and periodically recalibrate data-driven
tools.
